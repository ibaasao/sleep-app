...
const BINAURAL_BASE_FREQ_HZ = 70;
...
export function HealingToneButton() {
  ...
  const startPlayback = useCallback(async () => {
    ...
    if (currentSound.noiseType) {
      ...
      const oscL = new Tone.Oscillator(currentSound.freq!, "triangle").connect(
        lowpassL,
      );
      ...
      oscL.volume.value = -22;
      oscR.volume.value = -22;
      sourceRef.current = oscL;
      secondOscRef.current = oscR;
    } else {
      const f = currentSound.freq!;
      const osc = new Tone.Oscillator(f, "triangle").connect(reverb);
      osc.volume.value = -14;
